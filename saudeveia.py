...
if aba == "Estoque":
        ...
        for _, r in df.iterrows():
            ...
            if st.session_state.admin:
                with st.expander(f"⚙️ Ajustar {r['nome']}"):
                    # CORREÇÃO: Limite de preço aumentado para evitar erro de Valor Máximo
                    v_add = st.number_input("Adicionar Qtd", 0.0, 10000.0, 30.0, key=f"add_{r['id']}")
                    ...
                    if st.button("Confirmar Ajuste", key=f"btn_{r['id']}", use_container_width=True):
                        ...
                        if res_p.status_code in [200, 201, 204]:
                            ...
                            st.error(f"Erro ao salvar. Status: {res_p.status_code}")
    ...
    total_rem = df_r['preco'].sum() if not df_r.empty else 0
    total_con = df_c['valor'].sum() if not df_c.empty else 0
    
    col1.metric("Total Remédios", f"R$ {total_rem:,.2f}")
    col2.metric("Total Consultas", f"R$ {total_con:,.2f}")

    f_gastos = []
    if not df_r.empty:
        temp_r = df_r[['data_inicio', 'preco']].copy().rename(columns={'data_inicio': 'Data', 'preco': 'valor'})
        temp_r['Tipo'] = 'Remédios'; f_gastos.append(temp_r)
    if not df_c.empty:
        temp_c = df_c[['data_consulta', 'valor']].copy().rename(columns={'data_consulta': 'Data'})
        temp_c['Tipo'] = 'Consultas'; f_gastos.append(temp_c)
    ...
    if st.session_state.admin:
        ...
        with st.form("form_cad", clear_on_submit=True):
            if tipo == "Remédio":
                ...
                f_qtd = st.number_input("Quantidade Total", 0.0, step=0.5)
                f_dose = st.number_input("Dose Diária", 0.0, step=0.5)
                f_preco = st.number_input("Preço R$", 0.0, max_value=1000000.0)
                if st.form_submit_button("SALVAR"):
                    if f_nome and f_qtd > 0:
                        pay = {"nome": f_nome, "qtd_total": float(f_qtd), "dose_diaria": float(f_dose), "preco": float(f_preco), "data_inicio": dt_iso}
                        r = requests.post(f"{URL_BASE}remedios", headers=HEADERS, json=pay)
                        if r.status_code in [200, 201, 204]:
                            st.success("✅ Salvo!"); enviar_telegram(f"🆕 {f_nome} cadastrado.")
                            st.cache_data.clear(); time.sleep(1); st.rerun()
                        else: st.error(f"Erro {r.status_code}. Tente um nome mais curto.")
            else:
                f_med = st.text_input("Médico")
                f_vlr = st.number_input("Valor R$", 0.0, max_value=1000000.0)
    ...
    if st.session_state.admin:
        tab_rem = st.selectbox("Tabela:", ["remedios", "consultas"])
        df_rem = buscar_dados(tab_rem)
        if not df_rem.empty:
            col_name = 'nome' if tab_rem == 'remedios' else 'medico'
            item_rem = st.selectbox("Item:", df_rem[col_name].tolist())
